...
from tasks import process_gtfs_routes , process_gtfs_stops , process_gtfs_agency , process_gtfs_calendar_dates , process_gtfs_calendars , process_gtfs_trips
...
async def gtfsImporter(file : UploadFile = File(...) , db : Session = Depends(get_db)):
    ...
    try:
        ...
        import_status.task_id = f"{routes_task.id},{stops_task.id},{agency_task.id},{calendar_dates_task.id},{calendar_task.id},{trips_task.id}" 
        db.commit()
